...
open class XViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    fun setImageResource(viewId: Int, imageResId: Int) = also { imageView(viewId).setImageResource(imageResId) }
    fun setImageDrawable(viewId: Int, drawable: Drawable?) = also { imageView(viewId).setImageDrawable(drawable) }
    fun setImageBitmap(viewId: Int, bitmap: Bitmap?) = also { imageView(viewId).setImageBitmap(bitmap) }
    fun setBackgroundColor(viewId: Int, color: Int) = also { viewById(viewId).setBackgroundColor(color) }
    fun setBackgroundResource(viewId: Int, backgroundRes: Int) = also { viewById(viewId).setBackgroundResource(backgroundRes) }
    fun setVisibility(viewId: Int, isVisible: Boolean) = also { viewById(viewId).visibility = if (isVisible) View.VISIBLE else View.INVISIBLE }
    fun setGone(viewId: Int, isGone: Boolean) = also { viewById(viewId).visibility = if (isGone) View.GONE else View.VISIBLE }
